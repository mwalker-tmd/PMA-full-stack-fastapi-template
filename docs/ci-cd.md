...
All CI workflows use **test-safe environment variables** that are hardcoded in the workflow files. These are NOT production secrets and are safe to commit to the repository.
...
### Why hardcode test values?

1. **Works out of the box**: No GitHub secrets configuration needed
2. **Template-friendly**: When someone forks/clones, tests run immediately
3. **Simple for students**: No setup required for bootcamp participants
4. **Isolated**: These values are ONLY used in CI, never in production
...
| **CI/GitHub Actions** | Hardcoded in workflows | Hardcoded in workflows | `.github/workflows/*.yml` |
| **Production** | Server environment | Server environment | Server `.env` or secrets manager |

### Do I need to set GitHub Secrets?

**For CI tests**: ❌ No! The workflows already have test values.

**For deployment (CD)**: ✅ Yes, you'll need production secrets (see [Deployment Guide](../deployment.md)).
